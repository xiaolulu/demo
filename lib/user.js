	...
	var config = {
		...
		callback = function( ret ){
			...
			if( ret.code == 0 ){
				...
				req.body = {'openId': toy.getCookie( req.headers.cookie, 'userid' )}
				bindOpenid( req, res ); 
			}
			res.send( ret );
...
function bindOpenid( req, res ){
	console.log( 'bindOpenid' + req.body.openId );
	var config = {
			path: '/cif/front/user/bind'
		},
		callback = function( ret, resule, fields ){
			toy.log.info( 'bindOpenid: openid=' + req.body.openId + ';ret=' + JSON.stringify( ret ) );
		}
